...
- Source and local setup: https://github.com/earlyaidopters/away-together
- Full prompt: [TRAIN-MY-SPECIALIST.md](../prompts/TRAIN-MY-SPECIALIST.md)

The public demo needs no model download. It uses saved outputs from the real local models. Changes to budgets, requirements and photo selections recompute the application rules in your browser. It does not run fresh AI inference. The source repository includes the local version for that.
...
Follow docs/SETUP.md in the repository. Download the current companion-v2 release, run the manifest-checked restoration tool, install the text environment and frontend, and start the agency on localhost:8765. The current release supplies V2 and its active-model pointer.

Run one holiday and inspect the fresh receipt. Add the optional photo service only after the text path works. The photo model has a separate environment and download. Do not expose these local development services directly to the internet.

No Jev account is needed for local inference. Repeating the hosted Jev benchmark is separate and may incur charges. Model downloads, coding-assistant use, hardware and electricity are not free merely because inference runs locally.

## 9. Adapt one thing at a time

Run travel successfully first. Then use a separate workspace for your new task. Change the questions and candidate meanings, build checked examples, evaluate the starting model, and train only if the results justify it.
...
| Problem | Next step |
|---|---|
| Missing model | Restore the matching companion-v2 ZIP and verify the manifest. |
